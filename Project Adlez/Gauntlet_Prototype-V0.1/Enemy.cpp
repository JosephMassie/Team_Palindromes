...
bool Enemy::moveCheck()
{
	...
	V2DF pen(1,1); // initialized to 1,1 for the collision functions
	...
	if(m_room->collImpassable(checkRect, m_pos, &pen, TOP))
	{
		// if the collision is on the right side
		// make sure to reset penetration
		pen = V2DF(1,1);
		checkRect = getSubRect(RIGHT);
		if(m_room->collImpassable(checkRect, m_pos, &pen, RIGHT))
		{
		...
		if(m_room->collImpassable(checkRect, m_pos, &pen, LEFT))
		{
			// augment x position by the x componet of the penetration vector
			m_pos.x -= pen.x;
		}
		...
		if(m_room->collImpassable(checkRect, m_pos, &pen, BOT))
		{
			// augment x position by the x componet of the penetration vector
			m_pos.y -= pen.y;
		}
	}
	// now check against any enemies
	pen = V2DF(1,1);
	checkRect = getBoundRect();
	if( m_room->collEnemies(checkRect,0.0f, m_pos, &pen, RIGHT) )
	{
		// if the collision is on the right side
		// make sure to reset penetration
		pen = V2DF(1,1);
		checkRect = getSubRect(RIGHT);
		if(m_room->collEnemies(checkRect, 0, m_pos, &pen, RIGHT))
		{
			// augment x position by the x componet of the penetration vector
			m_pos.x -= pen.x;
		}
		...
		pen = V2DF(1,1);
		checkRect = getSubRect(LEFT);
		if(m_room->collEnemies(checkRect, 0, m_pos, &pen, LEFT))
		{
			// augment x position by the x componet of the penetration vector
			m_pos.x -= pen.x;
		}
		...
		pen = V2DF(1,1);
		checkRect = getSubRect(TOP);
		if(m_room->collEnemies(checkRect, 0, m_pos, &pen, TOP))
		{
			// augment x position by the x componet of the penetration vector
			m_pos.y -= pen.y;
		}
		...
		pen = V2DF(1,1);
		checkRect = getSubRect(BOT);
		if(m_room->collEnemies(checkRect, 0, m_pos, &pen, BOT))
		{
			// augment x position by the x componet of the penetration vector
			m_pos.y -= pen.y;
		}
...
void Enemy::update(float dT)
{
	...
	if(m_type != BOSS)
	{
		...
		else if (!m_player->getPosition().isWithin(atkRange, m_pos))
		{
			...
			V2DF avoid(0,0);
			if(m_room->wallWithin(m_pos,20,avoid))
			{
				if(!wandering)
					velocity.add(avoid.normal().product(1000));
				else
					steeringForce.add(avoid.normal().product(1000));
			}
			if(wandering)
